...
`human` 和 `auto` 都先运行相同的 VLM；只决定未决事件的处理方式。两者都可能因包含率冲突暂停，故应在交互终端运行。页面点击选项会生成答案，将其复制到当前运行终端并回车，只有当前题及冻结快照的编号有效。人工复核页保留当前题和浏览器草稿，不自动跳题。
...
- 仅在最终选择 KEEP_SEPARATE 时计算完整点云的双向包含率：一个点到另一个完整点云最近点距离不超过当前建图体素边长（默认 0.01 m）即命中。任意方向严格大于 90% 强制人工复核，auto 也不例外；不会据此直接合并。正好 90% 不触发。
