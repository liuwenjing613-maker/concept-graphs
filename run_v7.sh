...
if [[ "$MODE" != human && "$MODE" != auto ]]; then
  ...
  echo "Native candidates retain the >90% override; supplemental candidates require VLM votes."
  exit 2
fi
shift
...
exec "$PYTHON" "$ROOT/scripts/run_blocking_association_gate.py" --mode vlm --fallback "$MODE" --exp-suffix "v7_merge_2_${MODE}_$(date +%Y%m%d_%H%M%S)_$$" "$@"
