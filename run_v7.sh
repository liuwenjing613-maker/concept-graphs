...
if [[ "$MODE" != human && "$MODE" != auto ]]; then
  ...
  echo "Only current valid identity DIFFERENT can use >90% containment; fallback and locked events cannot."
  exit 2
fi
shift
...
exec "$PYTHON" "$ROOT/scripts/run_blocking_association_gate.py" --mode vlm --fallback "$MODE" --exp-suffix "v7_merge_re_${MODE}_$(date +%Y%m%d_%H%M%S)_$$" "$@"
