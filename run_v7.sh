...
if [[ "$MODE" != human && "$MODE" != auto ]]; then
  ...
  echo "Containment override uses quality veto, mutual coverage, then fragment resolution."
  exit 2
fi
shift
...
exec "$PYTHON" "$ROOT/scripts/run_blocking_association_gate.py" --mode vlm --fallback "$MODE" --exp-suffix "v7_merge_v3_${MODE}_$(date +%Y%m%d_%H%M%S)_$$" "$@"
