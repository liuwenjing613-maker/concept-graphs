...
This directory is the concrete evidence snapshot of the server run
`ali_my_evidence_smoke_20260817`, completed on `2026-08-17`, using Replica `room0`
with two processed frames (`start=0,end=40,stride=20`) on the ali-my branch.

Included are the exact JSON/JSONL records and the NPZ similarity/observation point-cloud
sidecars generated by that run, plus the two saved Hydra configuration files. The original
Replica frames, detector cache, model checkpoints, and server logs are intentionally not
copied into GitHub. Paths in the manifest and reference fields preserve the original server
provenance; use the server experiment directory when resolving external references.

`SHA256SUMS` covers every file in this snapshot except itself.
