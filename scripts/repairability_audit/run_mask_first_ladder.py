#!/usr/bin/env python3
"""Build a mask-first cumulative oracle ladder from an empty map.

Every condition consumes frames in timestamp order and starts from an empty map.
Later mask stages supersede earlier geometry representations instead of
re-inserting the same raw support.  The last stage changes association only.
"""
...
def update_observation_stream_hash(
    digest,
    *,
    raw_frame: int,
    ordinal: int,
    ...
    """Hash the effective per-frame mask stream before association."""
    digest.update(
        np.asarray(
            [raw_frame, ordinal, -1 if gt_id is None else int(gt_id)],
            dtype=np.int64,
        ).tobytes()
    )
...
MODE_DEFINITIONS = {
    "mf_op_native": (
        "Mask-first stage 1: normal processed observations plus owner-identifiable "
        "raw proposals rejected before valid 3D observation creation. All observations "
        "use frozen native online association; no GT identity is forced."
    ),
    "mf_om_pure_native": (
        "Mask-first stage 2: OP is subsumed by one GT-clipped union per "
        "owner-identifiable object from all raw proposals. The cleaned stream uses "
        "frozen native online association; earlier masks are not duplicated."
    ),
    "mf_om_all_native": (
        "Mask-first stage 3: OP and OM_pure are subsumed by maximal GT-clipped "
        "partition of every overlapping raw proposal with FP suppression. The stream "
        "still uses frozen native online association."
    ),
    "mf_om_all_oa": (
        "Mask-first stage 4: exactly the stage-3 OM_all observation stream followed "
        "by GT identity association, isolating residual association contribution."
    ),
}

MASK_FIRST_CAPABILITIES = {
    "mf_op_native": ["OP"],
    "mf_om_pure_native": ["OP", "OM_pure"],
    "mf_om_all_native": ["OP", "OM_pure", "OM_all"],
    "mf_om_all_oa": ["OP", "OM_pure", "OM_all", "OA"],
}

ASSOCIATION_POLICIES = {
    "mf_op_native": "frozen_native_online",
    "mf_om_pure_native": "frozen_native_online",
    "mf_om_all_native": "frozen_native_online",
    "mf_om_all_oa": "oracle_gt_identity",
}

PARTITION_MODE = {
    "mf_om_pure_native": "om_pure",
    "mf_om_all_native": "om_all",
    "mf_om_all_oa": "om_all",
}
...
def run_self_test() -> dict:
    ...
    assert {item["gt_id"] for item in pure}.issubset(
        {item["gt_id"] for item in maximal}
    )
    assert MASK_FIRST_CAPABILITIES["mf_om_all_oa"] == [
        "OP", "OM_pure", "OM_all", "OA"
    ]
    toy_gobs = {
        "mask": masks,
...
def main() -> int:
    ...
    if args.detections_root is None:
        parser.error("all mask-first modes require --detections-root")
    if args.mode == "mf_op_native" and args.baseline_map is None:
        parser.error("mf_op_native requires --baseline-map for post-run provenance audit only")
    ...
    effective_observation_digest = hashlib.sha256()
        ...
        if args.mode == "og":
            ...
            if args.mode == "mf_op_native":
                ...
                for detection in detections:
                    ...
                    processed_keys.add(observation_key(color_path, mask_idx))
                    assignment = mask_assignment(
                    ...
                    processed_assignments.append(
                        {
                            **assignment,
                            ...
                            "route": "frozen_native_online",
                        }
                    )
                    update_observation_stream_hash(
                        effective_observation_digest,
                        raw_frame=raw_frame,
                        ordinal=source_idx,
                        gt_id=assignment["gt_id"] if assignment["eligible"] else None,
                        mask=np.asarray(detection["mask"][0]),
                    )
                ...
                if specs:
                    ...
                    for detection in additions:
                        ...
                        update_observation_stream_hash(
                            effective_observation_digest,
                            raw_frame=raw_frame,
                            ordinal=len(raw_masks) + int(spec["gt_id"]),
                            gt_id=int(spec["gt_id"]),
                            mask=np.asarray(detection["mask"][0]),
                        )
                        native_detections.append(detection)
                        frame_counts["raw_additions_native"] += 1
                native_objects = native_ingest(native_objects, native_detections, cfg)
            else:
                partition_mode = PARTITION_MODE[args.mode]
                ...
                partitioned_native = MapObjectList(device=str(cfg.device))
                if specs:
                    synthetic = make_synthetic_gobs(raw_gobs, specs)
                    ...
                    for detection in partitioned:
                        ...
                        update_observation_stream_hash(
                            effective_observation_digest,
                            raw_frame=raw_frame,
                            ordinal=int(detection["mask_idx"][0]),
                            gt_id=int(spec["gt_id"]),
                            mask=np.asarray(detection["mask"][0]),
                        )
                        ...
                        if args.mode == "mf_om_all_oa":
                            detection["id"] = uuid.uuid5(
                                uuid.NAMESPACE_URL,
                                f"ladder-om_all:{sequence}:{color_path.stem}:{spec['gt_id']}",
                            )
                            merge_by_token(
                                oracle_objects,
                                token_to_index,
                            ...
                            frame_counts["partitioned_oracle_identity"] += 1
                        else:
                            partitioned_native.append(detection)
                            frame_counts["partitioned_native"] += 1
                if args.mode != "mf_om_all_oa":
                    native_objects = native_ingest(
                        native_objects, partitioned_native, cfg
                    )
    ...
    if processed_assignments:
        assignment_summary = {
            ...
            "owner_eligible_processed_masks": int(
                sum(row["eligible"] for row in processed_assignments)
            ),
            "native_online_masks": len(processed_assignments),
    ...
    manifest = {
        ...
        "oracle_and_unassigned_native_maps_isolated": False,
        "cumulative_capabilities": MASK_FIRST_CAPABILITIES[args.mode],
        "association_policy": ASSOCIATION_POLICIES[args.mode],
        "earlier_mask_geometry_duplicated": False,
        "effective_observation_stream_sha256": effective_observation_digest.hexdigest(),
        "seed": args.seed,
        "purity_threshold": args.purity_threshold,
        "min_overlap_pixels": args.min_overlap_pixels,
        "input_paths": {
