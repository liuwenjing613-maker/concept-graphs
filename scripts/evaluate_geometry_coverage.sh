...
# Usage: bash evaluate_geometry_coverage.sh v4_human2000
...
OUT="$TARGET/geometry_coverage_fixed_support_$(date +%Y%m%d_%H%M%S_%N)"
test -f "$MAP"
mkdir -- "$OUT"

HF_HUB_OFFLINE=1 /usr/bin/time -v "$ROOT/envs/cg-main/bin/python" \
  "$PROJECT/scripts/eval_replica_geometry_coverage.py" \
  ...
  --distances-m 0.025 0.05 0.10 \
  2>&1 | tee "$OUT/evaluation.log"

printf '\n覆盖率评估结果目录：%s\n' "$OUT"
cat "$OUT/coverage.csv"
