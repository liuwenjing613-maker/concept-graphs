    ...
    if dbscan_remove_noise and run_dbscan:
        if not return_stats:
            ...
            if after_downsample_points and ordered:
                ...
                stats["second_cluster_ratio"] = (
                    float(ordered[1][1] / after_downsample_points)
                    if len(ordered) > 1
                    else 0.0
                )
                if len(ordered) > 1:
...
def merge_obj2_into_obj1(
    ...
    custom_handled = [
        ...
        "revision_provenance_lineage_uids",
        "revision_identity_uids",
    ...
    if any(key in obj for obj in (obj1, obj2) for key in identity_keys):
        from conceptgraph.revision.identity import merge_identity_metadata

        # Provenance is monotonic; effective aliases are unioned only after the
        # caller has accepted this merge.
        merge_identity_metadata(obj1, obj2)
...
def compute_overlap_matrix_general(
    objects_a: MapObjectList, objects_b=None, downsample_voxel_size=None
) -> np.ndarray:
    """
...
def filter_gobs(
    ...
    for mask_idx in range(len(gobs["xyxy"])):
        ...
        if mask_area < mask_threshold:
            reject(
                "mask_area_below_threshold", int(mask_area), ">=", float(mask_threshold)
            )
            logging.debug(
        ...
        evaluated_gates.append(
            {
                "gate": "background_class",
                "value": class_name,
                "operator": "not_in",
                "threshold": list(BG_CLASSES or []),
                "passed": True,
            }
        )
        ...
        if class_name not in BG_CLASSES:
            ...
            if (
                ...
                reject(
                    "bbox_area_above_ratio",
                    float(bbox_area / image_area),
                    "<=",
                    float(max_bbox_area_ratio),
                )
                logging.debug(
            ...
            evaluated_gates.append(
                {
                    "gate": "bbox_area_ratio",
                    "value": float(bbox_area / image_area),
                    "operator": "<=",
        ...
        if mask_conf_threshold is not None and gobs["confidence"] is not None:
            if gobs["confidence"][mask_idx] < mask_conf_threshold:
                reject(
                    ...
                    float(gobs["confidence"][mask_idx]),
                    ">=",
                    float(mask_conf_threshold),
                )
            ...
            evaluated_gates.append(
                {
                    "gate": "confidence",
                    "value": float(gobs["confidence"][mask_idx]),
                    "operator": ">=",
                    "threshold": float(mask_conf_threshold),
                    "passed": True,
                }
            )
    ...
    filtered_captions = filter_captions(
        gobs["captions"], gobs["detection_class_labels"]
    )
    ...
    for mask_idx in range(len(gobs["mask"])):
        ...
        detected_object = {
            ...
            "inst_color": np.random.rand(
                3
            ),  # A random color used for this segment instance
