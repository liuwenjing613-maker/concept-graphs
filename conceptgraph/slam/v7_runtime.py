...
def parse_stage(content,task,labels=None):
    ...
    if task=='observation_quality':
        if mode!='STRICT_JSON':raise ValueError('observation quality requires strict JSON')
        errors={'none','mixed_objects','background_leak','severe_truncation','too_small','blurry','heavy_occlusion','wrong_target','other'}
        if (set(value)!={'quality','confidence','reason','error_type'}
            or value['quality'] not in {'USABLE','INVALID','UNCERTAIN'}
            or type(value['confidence']) is not int or not 0<=value['confidence']<=5
            or not isinstance(value['reason'],str) or not 1<=len(value['reason'])<=120
            or value['error_type'] not in errors):
            raise ValueError('invalid observation quality schema')
        if sum('\u4e00'<=c<='\u9fff' for c in value['reason'])>20:
            raise ValueError('reason exceeds 20 Chinese characters')
        if (value['quality']=='USABLE')!=(value['error_type']=='none'):
            raise ValueError('quality/error_type contradiction')
        value=dict(status={'USABLE':'USABLE','INVALID':'CORRUPTED','UNCERTAIN':'INSUFFICIENT'}[value['quality']],
                   reason=value['reason'])
...
class V7Runtime(VLMRuntime):
    ...
    def __init__(self,owner):
        ...
        self.endpoint_pool=EndpointPool(json.loads(os.environ.get('V7_VLM_URLS', json.dumps([owner.base_url]))), owner.timeout_seconds,max_timeout_retries=0)
        self.owner=owner;self.root=owner.output_dir;self.rows={};self.projection_frames={}
        self.request_cache=EvidenceRequestCache(self.root/'request_cache')
        self.status='running';self.prompts={}
        self.root.joinpath('review').mkdir(exist_ok=True)
        self.evidence=LiveEvidence(self)
        self.versions=dict(version='v7_CLIP_no90_new',model=owner.model,fallback=self.fallback,
            execution_revision='20260913_clip_no90_new',
            endpoints=self.endpoint_pool.urls,max_parallel=len(self.endpoint_pool.urls),timeout_retries=0,
            timeout_failure_after=1,
            ...
            renderer='observation quality A/B revision_02 + focused-fivepanel + full-RGB-node-audit + target-RGB-history/RGB-projection/zoom',
            merge_verifications_per_evidence=1,reject_votes=False,containment_distance_m=self.containment_distance,
            containment_threshold=.9,auto_requires_human=False,
            defer_policy='PRESERVE_BASELINE_INCLUDING_MERGE',cache_policy='layered_evidence_and_exact_request_single_flight',
            containment_trigger='disabled in v7_CLIP_no90; KEEP_SEPARATE never receives geometry approval')
    ...
    def _pool(self):
        if not hasattr(self,'endpoint_pool'):
            self.endpoint_pool=EndpointPool([self.owner.base_url],self.owner.timeout_seconds,max_timeout_retries=0)
        return self.endpoint_pool

    def stage(self,event_id,directory,task,images,snapshot,labels=None,cache_context=None):
        return self.stage_many(event_id,[(directory,task,images,labels,cache_context)],snapshot)[0]

    def stage_many(self,event_id,specs,snapshot):
        # Prepare and publish on the mapping thread. Workers only touch their own files.
        if not specs:return []
        prepared=[self._prepare_stage(event_id,*spec[:3],snapshot,spec[3],spec[4] if len(spec)>4 else None) for spec in specs]
    ...
    def _prepare_stage(self,event_id,directory,task,images,snapshot,labels=None,cache_context=None):
        ...
        request_fingerprint=digest(dict(task=task,labels=labels,payload=payload,parser_version='no90_new_v1'))
        key_request=request_fingerprint
        if task=='node_quality' and cache_context is not None:
            # A/B is only a display role for this independent quality task.
            # Raw selected RGB/mask hashes + renderer version bind its visual evidence.
            protocol=copy.deepcopy(payload);protocol['messages'][1]['images']=[]
            key_request=digest(dict(task=task,labels=labels,payload=protocol,parser_version='no90_new_v1'))
        cache_key=digest(dict(request_fingerprint=key_request,evidence=cache_context))
        save_json(directory/'cache_key.json',dict(key=cache_key,request_fingerprint=request_fingerprint,
            evidence=cache_context,scope='run-local',snapshot_excluded_from_key=True))
        print('[v7-stage]',event_id,task,'START',directory.name,flush=True)
        return dict(directory=directory,task=task,snapshot=snapshot,labels=labels,payload=payload,request=request,cache_key=cache_key,request_fingerprint=request_fingerprint)

    def _invoke_stage(self,item):
        started=time.perf_counter()
        result,hit=self.request_cache.execute(item['cache_key'],lambda:self._invoke_uncached(item))
        result=copy.deepcopy(result)
        if hit:
            origin=dict(directory=result.get('directory'),h_snapshot_uid=result.get('h_snapshot_uid'),
                        seconds=result.get('seconds'),timing=result.get('timing'),
                        completed_at=result.get('completed_at'))
            result.update(cache_origin=origin,attempts=[],timeout_count=0,seconds=time.perf_counter()-started,
                          timing={},completed_at=_utc_now())
        result.update(task=item['task'],cache_hit=hit,cache_key=item['cache_key'],request_fingerprint=item['request_fingerprint'],
            h_snapshot_uid=item['snapshot'],c_bound_h_snapshot_uid=item['snapshot'],
            directory=str(item['directory'].relative_to(self.root)),images=item['request']['messages'][1]['images'],
            actual_call_count=0 if hit else len(result.get('attempts',[])))
        return result

    def _invoke_uncached(self,item):
    ...
    def fallback_choice(self,event_id,directory,task,images,reason,aliases,snapshot):
        # DEFER is abstention: preserve the mapper's original decision.
        choice='DEFER'
        save_json(directory/'auto_fallback.json',dict(reason=reason,choice=choice,
            policy='PRESERVE_BASELINE',h_snapshot_uid=snapshot))
        return choice
    def adjudicate(self,event_dir,candidates,snapshot,frame,source_frame,association_scores=None):
        ...
        if decision['kind']=='ASSOCIATE':choice=decision['target_alias']
            ...
            same=decision['same_aliases']
            if association_scores is None or any(a not in association_scores for a in same):
                raise EvidenceInvariantError('multi-SAME requires frozen mapper scores')
            scores={a:float(association_scores[a]) for a in same}
            if not all(np.isfinite(v) for v in scores.values()):
                raise EvidenceInvariantError('non-finite mapper association score')
            # Candidate order breaks exact ties; VLM confidence is never a score.
            choice=max(same,key=lambda a:scores[a])
            decision.update(kind='ASSOCIATE',target_alias=choice,
                reason_code='MULTIPLE_SAME_HIGHEST_MAPPER_SCORE',
                association_scores=scores,merge_execution='BASELINE_PROPOSALS_ONLY_AFTER_OBSERVATION_FUSION')
        elif quality['value'] and quality['value']['status']=='CORRUPTED':
    ...
    def flush_groups(self,objects,matches,cfg,evidence,frame,map_edges):
        if self.forced_groups:
            raise EvidenceInvariantError('forced merge groups are forbidden in v7_CLIP_no90_new')
