...
def evidence_safe(default=None):
    """Keep evidence failures from affecting the mapping algorithm."""
...
class EvidenceRecorder:
    ...
    def _initialize_storage(self, scene_id: str) -> None:
        for directory in (self.similarity_dir, self.observation_pcd_dir):
        ...
        self.manifest = {
            ...
            "end_time": None,
            "status": "running",
            "mapping_config_ref": self._relative(self.exp_out_path / "config_params.json"),
            "detection_config_ref": self._relative(
        ...
        for obs in snapshots:
            ...
            if kept:
                ...
                if save_pcd:
                    ...
                    pcd_ref = self._relative(pcd_path)

            detection_ref = obs.get("detection_ref")
            selector = f"[{raw_index}]"
            record = {
                "frame_uid": frame_uid,
                "obs_uid": obs["obs_uid"],
                ...
                "mask_ref": f"{detection_ref}/mask.npz#arr_0{selector}"
                if detection_ref
                else None,
                "crop_ref": f"{detection_ref}/image_crops.pkl.gz#{selector}"
                if detection_ref
                else None,
                "image_feat_ref": f"{detection_ref}/image_feats.npz#arr_0{selector}"
                if detection_ref
                else None,
                "text_feat_ref": f"{detection_ref}/text_feats.pkl.gz#{selector}"
                if detection_ref
                else None,
                "pcd_ref": pcd_ref,
                "n_points": n_points,
    ...
    def record_associations(
        ...
        similarity_ref = self._relative(similarity_path)
        ...
        for det_index, (detection, match_index) in enumerate(
            ...
            self._append(
                "associations.jsonl",
                {
                    ...
                    "spatial_sim_ref": f"{similarity_ref}#spatial_sim",
                    "visual_sim_ref": f"{similarity_ref}#visual_sim",
                    "aggregate_sim_ref": f"{similarity_ref}#aggregate_sim",
    ...
    def begin_vlm_context(self, **context) -> None:
        self._vlm_context = _plain(context)
        self._vlm_pending = []
